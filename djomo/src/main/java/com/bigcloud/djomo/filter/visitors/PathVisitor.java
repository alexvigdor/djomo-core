...
/**
 * Use PathVisitor.builder() to construct a PathVisitor with any number of other
 * FilterVisitors attached to path patterns.
 * 
 * @author Alex Vigdor
 *
 */
public class PathVisitor extends BaseVisitorFilter {
	protected final PathElement[] patterns;
	protected VisitorFilterFactory[] filters;
	...
	protected PathVisitor(PathFilter... pathFilters) {
		...
		VisitorFilterFactory[] fs = new VisitorFilterFactory[pathFilters.length];
	...
	private Visitor filterStack() {
		...
		for (int i = 0; i < psl; i++) {
			if (ps[i].matches(p)) {
				VisitorFilter vs = fs[i].newVisitorFilter();
				vs.filter(v);
				v = vs;
			}
		}
	...
	public void visitListItem() {
		filterStack().visitListItem();
		((ListElement) path).increment();
	}

	@Override
	public void visitObjectField(Object name) {
		((FieldElement) path).setName(name.toString());
		filterStack().visitObjectField(name);
	}
	...
	protected static class PathFilter {
		...
		final VisitorFilterFactory filter;

		protected PathFilter(String path, VisitorFilterFactory filter) {
	...
	public static class Builder {
		...
		public Builder filter(String path, VisitorFilterFactory filter) {
