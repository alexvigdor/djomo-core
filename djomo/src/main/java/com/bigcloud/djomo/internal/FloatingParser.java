 ...
 * 	-  adapted from "fastdoubleparser" library 
 *
...
public class FloatingParser {
	...
	/**
	 * The decimal exponent of a double has a range of -324 to +308. The hexadecimal
	 * exponent of a double has a range of -1022 to +1023.
	 */
	 ...
	 * {@literal (2^64-1) * 10^-342 = (2^64 - 1) * 2^-342 * 5^-342 > 2^−1074}. Thus,
	 * it is possible for a number of the form w * 10^-342 where w is a 64-bit value
	 * to be a non-zero double.
	 * <p>
	 * ********
	 * <p>
	 * If we are solely interested in the *normal* numbers then the smallest value
	 * is 2^-1022. We can generate a value larger than 2^-1022 with expressions of
	 * the form w * 10^-326. Thus, we need to pick SMALLEST_POWER_OF_TEN >= -326.
	 */
	final static int DOUBLE_MIN_EXPONENT_POWER_OF_TEN = -325;
	/**
	 * Largest power of 10 value of the exponent.
	 * <p>
	 * Any number of form w * 10^309 where {@literal w >= 1} is going to be infinite
	 * in a double, so we never need to worry about powers of 10 greater than 308.
	 */
	...
	/**
	 * When mapping numbers from decimal to binary, we go from w * 10^q to m * 2^p,
	 * but we have 10^q = 5^q * 2^q, so effectively we are trying to match w * 2^q *
	 * 5^q to m * 2^p.
	 * <p>
	 * Thus, the powers of two are not a concern since they can be represented
	 * exactly using the binary notation, only the powers of five affect the binary
	 * significand.
	 * <p>
	 * The mantissas of powers of ten from -308 to 308, extended out to sixty-four
	 * bits. The array contains the powers of ten approximated as a 64-bit mantissa.
	 * It goes from 10^{@value #DOUBLE_MIN_EXPONENT_POWER_OF_TEN} to
	 * 10^{@value #DOUBLE_MAX_EXPONENT_POWER_OF_TEN} (inclusively). The mantissa is
	 ...
	 */
	private static final double[] DOUBLE_POWERS_OF_TEN = { 1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11,
	 ...
	 * <p>
	 *
	 * @param str    a string containing a
	 *               {@code FloatingPointLiteralWithWhiteSpace}
	 * @param offset start offset of {@code FloatingPointLiteralWithWhiteSpace} in
	 *               {@code str}
	 ...
	 *         otherwise, {@code -1L}.
	 */
	public static final double parseNumber(char[] str, int offset, int length) {
		...
		if (hasLeadingZero) {
			ch = str[++index];
			if (ch == '0') {
				throw new NumberFormatException(new String(str, offset, length));
			}
		}
		...
		if ((ch | 0x20) == 'e') {// equals ignore case
			...
			do {
				...
				ch = str[++index];
			} while ((char) (ch - '0') < 10 && index < endIndex);
		...
		if (illegal || index < endIndex || !hasLeadingZero && digitCount == 0) {
			String val = new String(str, offset, length);
		...
		return Double.longBitsToDouble(valueOfFloatLiteral(str, offset, endIndex, isNegative, significand, exponent,
				isSignificandTruncated, exponentOfTruncatedSignificand));
	}

	private static int tryToParseEightDigits(char[] a, int offset) {
		long first = a[offset] | (long) a[offset + 1] << 16 | (long) a[offset + 2] << 32 | (long) a[offset + 3] << 48;
		long second = a[offset + 4] | (long) a[offset + 5] << 16 | (long) a[offset + 6] << 32
				| (long) a[offset + 7] << 48;
	...
	public static int tryToParseEightDigitsUtf16(long first, long second) {
		...
		return (int) (sval * 0x03e8_0064_000a_0001L >>> 48) + (int) (fval * 0x03e8_0064_000a_0001L >>> 48) * 10000;
	}
	 ...
	 *                                       literal (and maybe more)
	 * @param startIndex                     the start index (inclusive) of the
	 *                                       float literal inside the string
	 * @param endIndex                       the end index (exclusive) of the float
	 *                                       literal inside the string
	 * @param isNegative                     whether the float value is negative
	 * @param significand                    the significand of the float value (can
	 *                                       be truncated)
	 ...
	 *         otherwise, {@code -1L}.
	 */
	static long valueOfFloatLiteral(char[] str, int startIndex, int endIndex, boolean isNegative, long significand,
			int exponent, boolean isSignificandTruncated, int exponentOfTruncatedSignificand) {
		double d = tryDecFloatToDoubleTruncated(isNegative, significand, exponent, isSignificandTruncated,
	...
	static double tryDecFloatToDoubleTruncated(boolean isNegative, long significand, int exponent,
			boolean isSignificandTruncated, final int exponentOfTruncatedSignificand) {
		if (significand == 0) {
		...
		if (isSignificandTruncated) {
			...
			// To know whether rounding up is needed, we may have to examine up to 768
			// digits.
			...
			if (DOUBLE_MIN_EXPONENT_POWER_OF_TEN <= exponentOfTruncatedSignificand
				...
				double withoutRounding = tryDecToDoubleWithFastAlgorithm(isNegative, significand,
						exponentOfTruncatedSignificand);
				double roundedUp = tryDecToDoubleWithFastAlgorithm(isNegative, significand + 1,
						exponentOfTruncatedSignificand);
	 ...
	 * <p>
	 * This function will only work in some cases, when it does not work it returns
	 * NaN. This should work *most of the time* (like 99% of the time). We assume
	 * that power is in the [{@value #DOUBLE_MIN_EXPONENT_POWER_OF_TEN},
	 * {@value #DOUBLE_MAX_EXPONENT_POWER_OF_TEN}] interval: the caller is
	...
	static double tryDecToDoubleWithFastAlgorithm(boolean isNegative, long significand, int power) {
		...
		// this will be non-zero because the most significant bit of shiftedSignificand
		// is 1.
