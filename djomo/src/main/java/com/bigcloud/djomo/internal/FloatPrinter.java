...
public class FloatPrinter {
	...
	/**
	 * Callers must assure buffer has 15 characters available before calling
	 * 
	 * @param val
	 * @param buf
	 * @param index
	 * @return the new ending index of written data
	 */
	public static int printFloat(float val, char[] buf, int index) {
		...
		if (scientificNotation) {
			...
			for (int i = 0; i < olength - 1; i++) {
				int c = output % 10;
				output /= 10;
				buf[index + olength - i] = (char) ('0' + c);
			}
			buf[index] = (char) ('0' + output % 10);
			buf[index + 1] = '.';
			index += olength + 1;
			if (olength == 1) {
				buf[index++] = '0';
			...
			if (exp < 0) {
				...
				for (int i = 0; i < olength; i++) {
					buf[index + olength - i - 1] = (char) ('0' + output % 10);
					output /= 10;
				}
				index += olength;
	 ...
	 * i.e., returns i such that value = 5^i * x, where x is an integer.
	 */
	...
	/**
	 * Compute the exact result of [m * 2^p / 10^q] = [m * 2^(p - q) / 5 ^ q] = [m *
	 * [2^k / 5^q] / 2^-(p - q - k)] = [m * POW5_INV[q] / 2^j].
	 */
